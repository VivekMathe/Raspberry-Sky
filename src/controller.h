...
private:
	...
	double max_angle;
	Vector3d Controller::outer_achievePos(const Vector3d& p_des, const Vector3d& v_des); //returns att + T
	Vector3d Controller::inner_achieveAtt(const Vector3d& att_cmd, const Vector3d& omega_cmd); //returns L M N
	Vector3d g;
	double m;
public:
